        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            if self.sapg2:
                if self.vanilla_sapg:
                    ...
                    a_loss_info = {
                        "ppo": a_loss.mean(),
                        "awac": torch.tensor(0.0, device=self.ppo_device),
                        "klppo": torch.tensor(0.0, device=self.ppo_device),
                    }
                else:
                    a_loss, a_loss_info = self.actor_loss_func(
                        old_action_log_probs_batch,
                        action_log_probs,
                        leader_action_log_probs,
                        advantage,
                        self.ppo,
                        curr_e_clip,
                        off_policy_mask,
